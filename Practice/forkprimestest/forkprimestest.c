...
// BRUTE FORCE METHOD to test a number is prime.
...
int main(int argc, char *argv[]) {
    ...
    pid_t pid;
    if ((pid = fork()) == 0) {
        // Child process that finds and displays primes forever.
        unsigned long num = 2;
    ...
    // Sleep for the specifies time and then kill the child process.
    sleep(sleep_secs);
